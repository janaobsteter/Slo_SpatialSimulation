...
for (year in 1:nYear) {
  rm(Gmatrix)
  # Read in the GRM
  load(paste0("RData/GRMAge0_", year, ".Rdata"))

  #1) Plot the relationships against the distance
  # Create a distance matrix
  distMatrix <- as.matrix(colonyRecords %>%  filter(queenID %in% rownames(Gmatrix)) %>%
                          .[match(rownames(Gmatrix), .$queenID),] %>% # Make sure the distance matrix is in the same order as GRM
                          `rownames<-`(.$queenID) %>%
  ...
  png(paste0("GRM/GrmDistance", year, ".png"))
  ...
  xOrderedId <- colonyRecords %>%  filter(queenID %in% rownames(Gmatrix)) %>%
    select(queenID, locationX) %>%  arrange(locationX) %>% select(queenID)
  # Order the Gmatrix by X
  xMatch <- match(as.character(xOrderedId$queenID), rownames(Gmatrix))
  ...
  png(paste0("GRM/xOrderedGRM_", year, ".png"))
  ...
  yOrderedId <- colonyRecords %>%  filter(queenID %in% rownames(Gmatrix)) %>%
    select(queenID, locationY) %>%  arrange(locationY) %>% select(queenID)
  # Order the Gmatrix by X
  yMatch <- match(as.character(yOrderedId$queenID), rownames(Gmatrix))
  ...
  png(paste0("GRM/yOrderedGRM_", year, ".png"))
  #print(image(as(xOrderedG, "Matrix"), main = paste0("Year", year)))
...
system(paste0("convert -delay 100 -loop 0 ", paste0("GRM/xOrderedGRM_", 1:nYear, ".png", collapse = " "), " GIFs/xOrderedGRM.gif"))
system(paste0("convert -delay 100 -loop 0 ", paste0("GRM/yOrderedGRM_", 1:nYear, ".png", collapse = " "), " GIFs/yOrderedGRM.gif"))
system(paste0("convert -delay 100 -loop 0 ", paste0("GRM/GrmDistance", 1:nYear, ".png", collapse = " "), " GIFs/GrmDistance.gif"))
