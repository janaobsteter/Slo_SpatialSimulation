...
data_rec <- function(datafile, colonies, year, proportionKnown) {
  ...
  datafile = rbind(datafile,
                   data.frame(colonies             = c(deparse(substitute(colonies))),
                              ...
                              proportionKnown      = proportionKnown,
  ...
  for (Rep in 1:nRep) {
    ...
    for (year in 1:nYear) {
      ...
      if (year == 1) {
        ...
        start = Sys.time()
        colonyRecords <- data_rec(datafile = colonyRecords, colonies = age1, year = year, proportionKnown = proportionKnown)
        end = Sys.time()
      ...
      colonyRecords <- data_rec(datafile = colonyRecords, colonies = age0, year = year, proportionKnown = proportionKnown)
      # Records phenotypes
